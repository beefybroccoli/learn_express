...
// here we validate the API key, by mounting this middleware to "/api" meaning only paths prefixed with "/api"
...
app.use(
  ...
  function (request, res, next) {
    ...
    // key isn't present
    if (!key) return next(cb_error(400, "api key required"));

    // key is invalid
    if (!~apiKeys.indexOf(key)) return next(cb_error(401, "invalid api key"));
