...
app.resource = function (path, object) {
  this.get(path, object.index);
  this.get(path + "/:a..:b.:format?", function (req, res) {
    var a = parseInt(req.params.a, 10);
    var b = parseInt(req.params.b, 10);
    var format = req.params.format;
    object.range(req, res, a, b, format);
  });
  this.get(path + "/:id", object.show);
  this.delete(path + "/:id", function (req, res) {
    var id = parseInt(req.params.id, 10);
    object.destroy(req, res, id);
  });
};
...
if (!module.parent) {
  //port 4016, resource - Multiple HTTP operations on the same resource
  app.listen(4016);
  console.log(
    "Express started on port 4016, resource - Multiple HTTP operations on the same resource"
  );
}
